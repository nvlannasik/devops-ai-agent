...
- **Hitting the deadline forces an answer; it does not abandon the run.** Until 2026-09-03 the deadline branch returned "⚠️ Investigation exceeded its time budget" straight from the top of the `while`, discarding every tool result gathered — the same regression the iteration ceiling was written to fix, still live here because no backend was slow enough to reach 300s. `LLM_API_FORMAT=agent-builder` answers in 20-100s per call and reaches it on an ordinary alert. The branch now injects `TIME_BUDGET_NOTICE` and spends one tool-free turn; `toolsDisabled` stops it being taken twice, and a run with `toolRounds === 0` still gets the apology because there is nothing to salvage.
- **Sizing the budget for a slow backend:** 300s divided by the per-call latency IS the real iteration limit, and on the agent-builder path that can be lower than `MAX_ITERATIONS = 10`. Raise `INVESTIGATION_TIMEOUT_SECONDS` for that backend rather than assuming ten rounds are available — the forced answer keeps a truncated run useful, but a run cut off at round three answers from three rounds of evidence.
...
20. **The log-gap nudge threw away correct answers** — the gate appends `LOG_GAP_NOTICE` and `continue`s, so whatever the model writes on the extra round REPLACES the answer that triggered it. On thread `1789488072` (2026-09-15) a correct "Tidak ada anomali… 59 pod di 14 namespace sehat" was replaced twice by the scope-refusal boilerplate ("That's outside what I do — I'm a DevOps agent for this cluster"), and the good answer never reached Slack. Two causes, both fixed: the extra round ran **no tools**, so it had learned nothing — `logGapAction()` returns `restore` for exactly that case — and the gate fired on a *mention* whose playbooks had been accumulated four questions earlier, so it now fires in `mode: "alert"` only. Covered by `log-gap.test.ts`.
21. **A thread's playbooks ratcheted to the cap and froze there** — `selectForThread` only ever adds. Turn 5 of the same thread filled all five slots with `rca-format, pod-pending, pod-not-ready, multi-pod-one-cause, resource-rightsizing`, and the seven turns after it ran on that set: "remediation skill kamu apa aja emangnya?" was investigated with `pod-pending`, and "investigasi kenapa prometheus query nya kosong" could not load a relevant playbook because there was no slot left. `decayThreadSkills()` now runs at the start of each turn and keeps only the `CARRIED_SKILLS` (2) most recent — and never a mode-keyed skill, which is re-selected from the tag on the same turn.
22. **Tool results had no age, so stale evidence was reported as current state** — a `tool_result` carries no timestamp and neither does the message around it. "cluster resource saat ini gimana?" at 16:44 was answered `59 pods in 14 namespaces` from a health scan run at **16:14**, with zero tool calls that turn; `groundingGaps` then flagged `cloudflared-sutansoluli`, a namespace no tool result in the thread had ever returned. Each tool round now ends with an `[EVIDENCE READ AT] <iso>` text block, `staleEvidenceNotice()` opens the next turn with the age of the freshest one, `buildTimeContext()` rides every turn instead of only the first, and `## Evidence Has an Age` in `prompts/system.md` is the rule. The stamp lives in the history rather than in a side map so it survives a pod restart with the conversation.
