...
| Level | When | Content |
|-------|------|---------|
| info  | once per `chat()` | chosen route, backend, attempt N of M |
...
The emphasised case is the only place the one-directional rule exists as something that can *fail*. Without
it the rule lives only in this document, and the next person who finds bidirectional failover "more robust"
will change it with nothing objecting.

### Manual verification (cannot be unit-tested)

Switching backends mid-investigation leaves `tool_use.id` values in history that were generated by a
different backend. History is provider-neutral `Message[]` / `ContentBlock[]`, so this is structurally
sound, but whether the Anthropic API accepts arbitrary `tool_use.id` strings on input must be verified once
against the real API during implementation. The repo's smoke driver is stubbed and offline and will not
catch it. **Do not treat this as a safe assumption.**
