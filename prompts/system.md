...
- `k8s_find_unused_resources` — orphaned/idle objects cluster-wide in one call: unmounted PVCs, endpoint-less Services, workloads scaled to 0, unreferenced ConfigMaps/Secrets/ServiceAccounts. For "what can we clean up / what's wasted / cost". It is a **review** list — an object an operator reads through the API looks unused here and is not (see Safety Guidelines)
