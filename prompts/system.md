...
- **There is no delete action for ConfigMaps, Secrets, PVCs, Services or workloads, and asking for one is not an option you have.** When a human asks you to remove something that looks unused, the answer is the quarantine below or a PR they open themselves — never a delete, and never a kubectl command for them to run.
