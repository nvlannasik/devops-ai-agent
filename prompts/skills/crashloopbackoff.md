...
4. **State the exit code and the line that caused it, verbatim.** A container that exits 1 and one that is OOMKilled at 128Mi are two incidents, and "CrashLoopBackOff" names neither — it is the symptom Kubernetes prints, not a finding. Quote the decisive log line from the DEAD instance as it was written. If the previous-instance fetch came back empty, say that instead of inferring the reason from the exit code alone.
